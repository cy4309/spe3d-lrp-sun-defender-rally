...
export default function UploadPage() {
  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden mx-auto max-w-mobile">
      ...
      <div className="relative z-10 flex min-h-dvh flex-col px-4 pb-32">
        ...
        <Signboard className="mb-2">
          <div className="px-4 pb-5 pt-4">
            ...
            <input ref={inputRef} type="file" accept="image/jpeg,image/png" capture="user" className="hidden" onChange={handleFileChange} />

            {/* Photo area */}
            <div className="relative mx-auto mb-4 w-full">
              {preview ? (
                <>
                  <div className="aspect-[4/5] overflow-hidden rounded-2xl bg-brand-blue">
                    <img src={preview} alt="預覽" className="h-full w-full object-cover" />
                  </div>
                  <img src={A.frame} alt="" className="pointer-events-none absolute inset-0 h-full w-full" aria-hidden />
                  <CloseButton onClick={clearFile} label="移除照片" variant="modal" />
                </>
              ) : (
                    ...
                    <div className="min-w-0 flex-1 pt-0.5">
                      <p className="text-[13px] font-medium leading-snug text-white">
                        ...
                        {"hasExample" in item && item.hasExample && (
                          <button type="button" onClick={() => setShowInstruction(true)} className="ml-1.5 inline-flex items-center gap-1 rounded-full border border-white/50 bg-white/10 px-2 py-0.5 text-[11px] font-normal text-white align-middle">
            ...
            <p className="w-full pl-4 mb-4 text-[11px] text-brand-orange">（須先同意使用條款才能繼續）</p>

            {error && <p className="mb-3 text-center text-xs text-red-600">{error}</p>}

            <button type="button" onClick={() => void handleUpload()} disabled={!canSubmit} className="upload-cta w-full disabled:cursor-not-allowed disabled:opacity-45">
              {uploading ? "上傳中…" : "開始製作"}
            </button>
          </div>
        </Signboard>
      ...
      <AssetImg src={A.bgCover} alt="" className="pointer-events-none absolute bottom-0 left-0 right-0 z-20 w-full" />
