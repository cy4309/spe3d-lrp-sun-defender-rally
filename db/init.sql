...
CREATE INDEX idx_pv_campaign_time ON page_views (campaign_id, viewed_at DESC);
CREATE INDEX idx_pv_campaign_user ON page_views (campaign_id, line_user_id);

-- 12. line_follow_events（乙方 webhook 轉發）--------------------------------
CREATE TYPE line_follow_event_type AS ENUM ('follow', 'unfollow');

CREATE TABLE line_follow_events (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    line_user_id    TEXT NOT NULL,
...
CREATE INDEX idx_lfe_campaign_time ON line_follow_events (campaign_id, occurred_at DESC);
CREATE INDEX idx_lfe_line_user ON line_follow_events (line_user_id, occurred_at DESC);
